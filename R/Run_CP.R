RunCPModel<-function(n_cp = 2,
  ...
  # Create a directory "fig" and "modeloutput" in current working directory
    ...
    dir.create(paste0("output", runnum), showWarnings = FALSE)
  ...
  if(n_cp == 2)
  {
    ...
    myinitial<-function(){list("alpha"=c(rnorm(2,0,3)),
                               ...
                               "cp.temp"=c(runif(1,(max(year.t/1000)-min(year.t/1000))*.706+0.01,(max(year.t/1000)-min(year.t/1000))*.7714-0.01),runif(1,(max(year.t/1000)-min(year.t/1000))*.8368+.01,(max(year.t/1000)-min(year.t/1000))*.9022-0.01)),                                           
                               "sigma_err"=runif(1,0,1) )}
  }
  ...
  ########Run the model########
    mod <- suppressWarnings(jags(data=jags_data,
                ...
                n.burnin=userburnin,
                n.thin=userthin))
    save(mod, file = file.path(paste0("output", runnum), paste0("jags_mod",cpmodel, ".Rdata")))
#    save(mod, file = file.path("output", paste0("jags_mod",cpmodel, ".Rdata")))
    save(jags_data, file = file.path(paste0("output", runnum), paste0("jags_data", ".Rdata")))
