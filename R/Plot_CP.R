                    ...
                    include_errors = FALSE)
{
    ...
    dir.create(paste0("fig", runnum), showWarnings = FALSE)
  ...
  ggsave(p, file = paste0('fig',runnum,'/dat_plot.pdf'), width = 10, height = 6)    
                   ...
                   title="")
{
  ...
  if(n_cp == 0)
  {
    ...
    load(paste0("output",runnum,"/jags_mod",0,".Rdata"))
...
# Create data frame for plotting
# df.s<-data.frame(sstar_mean,sstar_upr95,sstar_lwr95,sstar_upr90,sstar_lwr90,sstar_upr50,sstar_lwr50,jags_data$year.t*1000)
#                  #sstar_median,
# names(df.s)<-c("Y_mean","upr95","lwr95","upr90","lwr90","upr50","lwr50","Year")
#                #"Y_median",
    
#df.s<-data.frame(sstar_mean,sstar_upr,sstar_lwr,jags_data$year.t*1000)
#names(df.s)<-c("Y","upr","lwr","Year")

# Create estimate of change in RSL from -1300 to -900:
df.s<-data.frame(sstar_mean,sstar_upr,sstar_lwr,jags_data$year.t*1000)
names(df.s)<-c("Y","upr","lwr","Year")
...
# get ests
par_est <- get_ests(dat, runnum = runnum,
         n_cp = n_cp)
...
res<-ggplot()+
 ...
 # geom_ribbon(data=df.s,aes(ymin=lwr95,ymax=upr95,x=Year,colour="red"),alpha=0.1,colour="red")+

geom_point(data=dat,aes(y=y,x=year))+
  ylab(yaxis.lab)+
  ggtitle(title)+
  theme_minimal()
ggsave(res, file = paste0("fig", runnum, "/",saveas), width = 10, height = 6)
